...
const SignUpForm = ({ onSubmit }) => {
  ...
  const { isLoading, handleEmailSignUp } = useFirebaseAuth()
  ...
  const handleSubmit = (data) => {
    if (onSubmit) {
      handleEmailSignUp(data, onSubmit)
    }
  }
  ...
  return (
    <form onSubmit={form.handleSubmit(handleSubmit)} className="flex flex-col gap-6">
      ...
      <div className="my-2 flex w-300 items-center justify-between md:flex-col-reverse md:gap-4">
        ...
        >
          <span className="flex items-center justify-center gap-2 text-base text-white">
